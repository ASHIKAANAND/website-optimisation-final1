...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    var select=document.querySelector("#pizzaSize");
    switch(size) {
      case "1":
         select.innerHTML= "Small";
        return;
      case "2":
        select.innerHTML = "Medium";
        return;
      case "3":
        select.innerHTML = "Large";
        return;
      default:
  ...
  function changePizzaSizes(size) {
    ...
    for (var i = 0; i < pizzacon.length; i++) {
      pizzacon[i].style.width = newwidth + "%";//calculating new width adding %.
    }
  }
